...
public class D81 extends DiskImage {
	...
	public void readDirectory() {
		if (isCpmImage()) {
			...
			feedbackStream.append("partTracks: ").append(partMap.keySet().stream().sorted((s1, s2) -> s1.compareTo(s2)).map(String::valueOf).collect(Collectors.joining(","))).append('\n');
		}
	}
	...
	public byte[] getFileData(int number) throws CbmException {
		...
		} else if (cbmFile[number].getFileType() == FileType.CBM) {
			int blockPos = getSectorOffset(cbmFile[number].getTrack(), 0);
			int len = cbmFile[number].getSizeInBlocks() * BLOCK_SIZE;
